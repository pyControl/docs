...
Pyboard pins 'X5' and 'X6' support analog output.  On the breakout board they are connected to the BNC connectors labelled DAC-1 and DAC-2 and also to the special function pins on behaviour ports 3 and 4 respectively.  To use these pins as analog outputs see the [pyb.DAC](https://docs.micropython.org/en/latest/library/pyb.DAC.html) class in the micropython docs.
