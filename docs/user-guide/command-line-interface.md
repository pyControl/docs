...
pyControl may not be compatible with pyboards that are running old versions of the Micropython firmware (version < 1.9).  When you connect to a board with the CLI the micropython version running on the board is displayed.  Instructions for updating the micropython firmware can be found for windows [here](http://micropython.org/download) and for Linux/Mac [here](https://github.com/micropython/micropython/wiki/Pyboard-Firmware-Update).  The instructions say to put the board into device firmware update (DFU) mode by physically connecting the DFU pin to 3.3V while you press reset - an easier way to do this is using the board config menu in the *run_task* script.
